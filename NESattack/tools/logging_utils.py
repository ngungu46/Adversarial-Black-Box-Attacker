...
def log_output(sess, eval_logits, eval_preds, x, adv, initial_img, \
    ...
    imageio.imwrite(os.path.join(eval_dir, 'original.png'), (initial_img*255).astype(np.uint8))
    np.save(os.path.join(eval_dir, 'original.npy'), initial_img)
    imageio.imwrite(os.path.join(eval_dir, 'adversarial.png'), (adv*255).astype(np.uint8))
